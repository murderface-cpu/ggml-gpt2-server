...
The model weights (`models/`) are gitignored — they're large binaries and
...
— heavier, since it needs Python/PyTorch/transformers installed, which the
...
- **`docker compose up` fails to bind port 80/443** — something else on the
  ...
  reassign ports.
- **Backend not responding but Caddy is up** — `docker compose logs gpt2-api`.
