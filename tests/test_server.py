#!/usr/bin/env python3
"""
Black-box tests for the llama.cpp server (ghcr.io/ggml-org/llama.cpp:server)
run by docker-compose.yml, against a running instance. Talks plain HTTP with
the standard library only, no extra pip installs needed.
...
Usage:
    ...
    python3 tests/test_server.py --base-url http://vps-ip:8090
    python3 tests/test_server.py --api-key your-secret-key
...
def test_completions_streaming(base_url, api_key):
    ...
    ok = (
        len(chunks) >= 2
        and chunks[-1] == "[DONE]"
        and all(c == "[DONE]" or c.get("object") == "text_completion" for c in chunks)
    )
...
def test_invalid_json(base_url, api_key):
    ...
    try:
        ...
        raw = e.read().decode("utf-8")
        e.close()
    # llama.cpp surfaces a JSON parse error as 500 (not 400), but still with
    # a proper {"error": {...}} body — check it's rejected with an error,
    ...
    try:
        ...
        body = None
    ok = status >= 400 and body is not None and "error" in body
    report("POST /v1/completions with malformed JSON -> error", ok, f"status={status}")
...
def main():
    ...
    parser.add_argument("--base-url", default="http://127.0.0.1:8090", help="server base URL")
    parser.add_argument("--api-key", default=None, help="API key, if the server was started with one")
    args = parser.parse_args()

    print(f"testing llama.cpp server at {args.base_url}\n")
